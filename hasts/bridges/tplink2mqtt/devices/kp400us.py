...
class KP400US(TPLinkDevice):
    ...
    async def _check_outputs(self):
        ...
        # First Plug
        if self.always_publish or (self._kasa_device.get_plug_by_index(0).is_on != self._previous_outputs[0]):
            # State changed, so update previous and emit a message
            self._previous_outputs[0] = self._kasa_device.get_plug_by_index(0).is_on
        ...
        # Second Plug
        if self.always_publish or (self._kasa_device.get_plug_by_index(1).is_on != self._previous_outputs[1]):
            # State changed, so update previous and emit a message
            self._previous_outputs[1] = self._kasa_device.get_plug_by_index(1).is_on
            # Should the MAC address have the ':' characters removed?
            await self._mqtt_client.publish(
